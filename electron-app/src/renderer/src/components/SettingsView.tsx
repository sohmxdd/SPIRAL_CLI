...
import { Key, Bot, Save, Check, RefreshCw, Eye, EyeOff, ShieldCheck, AlertCircle } from 'lucide-react'
        ...
        <div className="bg-[#202024] border border-zinc-800 rounded-2xl p-5 space-y-4 shadow-sm">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex items-center space-x-3">
              <div className="flex-1 flex items-center justify-between px-3.5 py-2.5 bg-purple-950/30 border border-purple-800/50 rounded-xl text-xs text-purple-200 font-medium">
                <div className="flex items-center space-x-2">
                  <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                  <span>Groq (llama-3.3-70b-versatile)</span>
                </div>
                <span className="text-[10px] bg-purple-900/60 px-2 py-0.5 rounded text-purple-300">Supported</span>
              </div>
            </div>
            <p className="text-[11px] text-zinc-500 italic pt-0.5">
              Groq is the active backend provider. Additional providers will appear when backend SDKs are enabled.
            </p>
          </div>
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              <label className="text-xs font-medium text-zinc-300 flex items-center space-x-1.5">
                ...
                {apiKey ? (
                  <span className="text-[10px] text-emerald-400 bg-emerald-950/60 px-1.5 py-0.2 border border-emerald-800/40 rounded flex items-center space-x-1">
                    <ShieldCheck className="w-3 h-3 inline" />
                    <span>Encrypted at rest</span>
                  </span>
                ) : (
                  <span className="text-[10px] text-amber-400 bg-amber-950/60 px-1.5 py-0.2 border border-amber-800/40 rounded flex items-center space-x-1">
